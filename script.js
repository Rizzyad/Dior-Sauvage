...
function addReviewCard(name, product, comment) {
    ...
    const existingCardsCount = document.querySelectorAll('.review-card').length;

    // Tentukan kolom dan baris untuk card baru
    const column = existingCardsCount % 3;
    const row = Math.floor(existingCardsCount / 3);
    ...
    // Set tata letak menggunakan CSS Grid
    newReviewCard.style.gridColumn = `${column + 1}`;
    newReviewCard.style.gridRow = `${row + 1}`;
